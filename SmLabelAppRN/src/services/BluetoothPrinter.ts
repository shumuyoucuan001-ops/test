...
class BluetoothPrinterService {
  ...
  async disconnect(): Promise<void> {
    ...
    try {
      if (this.connectedDevice) {
        console.log('[BluetoothPrinter] 断开连接:', this.connectedDevice.name);
        await this.rnBluetoothClassic.disconnect();
        console.log('[BluetoothPrinter] 断开连接成功');
      }
    } catch (error) {
      console.error('[BluetoothPrinter] 断开连接失败:', error);
      // 不抛出错误，只是记录日志
    } finally {
      this.connectedDevice = null;
    }
  ...
  async writeRaw(tspl: string | Uint8Array): Promise<boolean> {
    try {
      const dataLength = typeof tspl === 'string' ? tspl.length : tspl.length;
      console.log('[BluetoothPrinter] 发送数据，数据长度:', dataLength);
      ...
      // 如果数据太大（>10KB），使用分块传输
      if (dataLength > 10240) {
        console.log('[BluetoothPrinter] 数据较大，使用分块传输');
        return await this.writeInChunks(tspl, writeToDevice, write);
      }
  ...
  // 分块传输大数据
  private async writeInChunks(data: string | Uint8Array, writeToDevice: any, write: any): Promise<boolean> {
    const chunkSize = 1024; // 1KB per chunk
    const totalLength = typeof data === 'string' ? data.length : data.length;
    const chunks = Math.ceil(totalLength / chunkSize);
    
    console.log(`[BluetoothPrinter] 分块传输: ${chunks} 块，每块 ${chunkSize} 字节`);
    
    for (let i = 0; i < chunks; i++) {
      const start = i * chunkSize;
      const end = Math.min(start + chunkSize, totalLength);
      const chunk = typeof data === 'string' ? data.slice(start, end) : data.slice(start, end);
      
      console.log(`[BluetoothPrinter] 发送第 ${i + 1}/${chunks} 块`);
      
      try {
        if (writeToDevice && this.connectedDevice?.address) {
          await writeToDevice.call(this.rnBluetoothClassic, this.connectedDevice.address, chunk);
        } else if (write) {
          await write.call(this.rnBluetoothClassic, chunk);
        } else {
          throw new Error('No write API');
        }
        ...
        console.error(`[BluetoothPrinter] 第 ${i + 1} 块发送失败:`, e);
        return false;
      }
    }
    
    console.log('[BluetoothPrinter] 分块传输完成');
    return true;
  }
}
