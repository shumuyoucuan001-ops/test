  ...
  const doSearch = async () => {
    ...
    try {
      ...
      const enhancedData = await Promise.all((res || []).map(async (item) => {
        try {
          const auditRecords = await labelDataApi.bySku(item.skuCode);
  ...
  const handlePrint = async (item: ExtendedLabelPrintItem, copies = 1) => {
    try {
      ...
      let renderData: any = {
        ...
        productSpec: item.productSpec || item.spec || "",
        prevMonth: (() => { 
          const now = new Date(); 
      ...
      const html = await templateApi.render(template.id, renderData);
    ...
    },
    {
      ...
      render: (_: any, record: ExtendedLabelPrintItem, index: number) => (
        <Select
          ...
          style={{ width: '100%' }}
        >
          {record.auditData && record.auditData.length > 0 && (
