...
export default function PermissionPage() {
  ...
  return (
    <div style={{ padding: 24 }}>
      <Card title="权限管理" extra={<Space><Button onClick={async()=>{ try { await aclApi.init(); message.success('已初始化表'); } catch { message.error('初始化失败'); } }}>初始化表</Button><Button onClick={async()=>{ try { const presets = [
      ...
      message.success('默认权限已写入'); load(); } catch { message.error('写入失败'); } }}>初始化默认权限</Button><Button type="primary" onClick={()=>onEdit()}>新增权限</Button></Space>}>
